...
export const ProgressionMode: React.FC<ProgressionModeProps> = ({
  ...
  const isLastChord = currentChordIndex >= progression.length - 1;
  ...
  useEffect(() => {
    ...
    const progressionTimer = setInterval(() => {
      setBeatCount(prev => {
        ...
        if (newBeat % 4 === 0) { // Every 4 beats (2 seconds)
  ...
  const handleChordSkip = useCallback((index: number) => {
    if (isPlaying || isCompleted) return;
  ...
  const getBeatIndicatorClass = useCallback((beat: number) => {
    const currentBeat = (beatCount % 4) + 1;
    return `progression-mode__beat ${
      beat === currentBeat && isPlaying ? 'progression-mode__beat--active' : ''
    } ${
  ...
  return (
    ...
    >
      ...
      <div className="sr-only">
        Progression mode: Practice chord progressions and transitions. 
        Follow along with the progression and maintain steady timing.
      </div>
      ...
      {timeLimit && timeRemaining !== null && (
        <div className="progression-mode__timer">
          ...
          <span 
            className={`progression-mode__timer-value ${
              isTimeWarning ? 'progression-mode__timer-value--warning' : ''
            }`}
          >
      ...
      <div className="progression-mode__instructions">
        ...
        <p className="progression-mode__description">
          {question.instructions || `Practice the ${getProgressionName()} progression. Follow the timing and chord changes.`}
        </p>
        <div className="progression-mode__pattern-name">
          {getProgressionName()}
        </div>
      </div>
          ...
          {progression.map((chord, index) => (
            ...
            >
              ...
              <span className="progression-mode__chord-name">
                {chord.rootNote}{chord.quality === 'minor' ? 'm' : ''}
              </span>
            </button>
          ))}
        </div>
        ...
        <div className="progression-mode__chord-visual">
          <ChordDisplay
            chord={{
              ...
              neckPosition: currentChord.neckPosition,
            }}
            size={size}
            showLabel={false}
        ...
        <div className="progression-mode__beat-indicator">
          ...
          <div className="progression-mode__beats">
            {[1, 2, 3, 4].map(beat => (
              <div
                key={beat}
                className={getBeatIndicatorClass(beat)}
      ...
      <div className="progression-mode__controls">
        <button
          ...
          className={`progression-mode__play-button ${
            isPlaying ? 'progression-mode__play-button--playing' : ''
          } ${
      ...
      {isCompleted && (
        <div 
          className="progression-mode__completion"
          role="alert"
          aria-live="assertive"
        >
          ...
          <div className="progression-mode__completion-text">
            Great job practicing the {getProgressionName()}. 
            You maintained the timing through all {progression.length} chords.
          </div>
        </div>
      )}
      ...
      <div id="progression-mode-instructions" className="sr-only">
        Progression mode practice question. Use the controls to start and stop the progression. 
        Follow along with the chord changes and maintain steady timing.
