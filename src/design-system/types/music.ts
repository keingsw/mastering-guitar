...
export type ChordQuality = 
  | 'maj' | 'major' | ''  // Major chords
  | 'm' | 'min' | 'minor' // Minor chords  
  | 'dim' | '°'           // Diminished
  | 'aug' | '+'           // Augmented
  | 'sus2' | 'sus4'       // Suspended
  | '7' | 'maj7' | 'm7'   // Sevenths
  | 'dim7' | 'aug7'       // Extended diminished/augmented
  | '9' | 'maj9' | 'm9'   // Ninths
  | '11' | '13';          // Extensions
...
export const isValidInterval = (interval: string): interval is Interval => {
  const validIntervals: Interval[] = [
    'R', 'm2', 'M2', 'm3', 'M3', 'P4', 'TT', 'P5', 'm6', 'M6', 'm7', 'M7', 'P8'
  ];
...
export const MusicValidationErrors = {
  INVALID_NOTE: (note: string) => `Invalid note name: "${note}". Must be one of: C, C#, Db, D, D#, Eb, E, F, F#, Gb, G, G#, Ab, A, A#, Bb, B`,
  INVALID_CHORD: (chord: string) => `Invalid chord symbol: "${chord}". Must follow pattern: NoteName + Quality (e.g., "Cmaj7", "Am", "F#dim")`,
  INVALID_INTERVAL: (interval: string) => `Invalid interval: "${interval}". Must be one of: R, m2, M2, m3, M3, P4, TT, P5, m6, M6, m7, M7, P8`,
