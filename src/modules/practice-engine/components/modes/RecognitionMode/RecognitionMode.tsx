...
import { createQuestionResult, validateAnswer } from "../../../services/scoring-system";
  ...
  return (
    ...
    >
      ...
      <div id={`${componentId}-description`} className="sr-only">
        Recognition mode: Look at the triad on the fretboard and identify its quality. Select one of the multiple choice
      ...
      {showFeedback && result && (
        ...
        >
          <div className="recognition-mode__feedback-header">
            <span className="recognition-mode__feedback-status">{result.isCorrect ? "✓ Correct!" : "✗ Incorrect"}</span>
            <span className="recognition-mode__feedback-points">+{result.points} points</span>
          </div>
          <p className="recognition-mode__feedback-message">{result.feedback}</p>
